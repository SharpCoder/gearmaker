...
function handler({
    ...
    const { teeth, pitch, bore = 6, pa = 14.5, thickness = 6 } = queryStringParameters;
...
function result(debug, requestId, payload, headers = {}) {
    if (debug) {
        ...
        return new Promise((resolve) => {
            axios.post(`http://${AWS_LAMBDA_RUNTIME_API}/2018-06-01/runtime/invocation/${requestId}/response`, payload, {
                headers: {
                    ...headers,
                }
            }).finally(resolve);
...
if (DEBUG) {
    ...
    (async function() {
         await axios.get(`http://${AWS_LAMBDA_RUNTIME_API}/2018-06-01/runtime/invocation/next`).then(async resp => {
